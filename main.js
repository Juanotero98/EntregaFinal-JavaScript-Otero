...
//FUNCION PARA MOSTRAR ARRAY DE MENU CON ASYNC Y AWAIT//
...
Swal.fire({
  ...
  let carritoItems = document.getElementsByClassName('carrito-items')[0].children
  ...
  for(let i= 0; i< carritoItems.length; i++){
    let item = carritoItems[i];
        ...
        productosComprados.push({ titulo, cantidad, precio });
  }
  sessionStorage.setItem('ultima compra', JSON.stringify(productosComprados));

  let carritoItemsContainer = document.getElementsByClassName('carrito-items')[0];
